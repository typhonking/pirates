...
int setSail(){
  ...
  void dock(){
    char input[9];
    for (int y=0; y<location[1]+1; y++){
      fscanf (fPorts, "%s", localPort);
    }
    rewind (fPorts);
    printf("\n\n\n\n\n\n\n\n\n\n\n");
    if (newlyArrived) printf("-------  docked in %s -------", localPort);
    else printf("-------  ship is ready to sail from %s  -------", localPort);
    newlyArrived=0;
